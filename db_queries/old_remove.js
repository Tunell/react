const remove = {
    ...
    compositeMaterial: (pool, ids) => {
        return pool.getConnection()
            .then( connection => {
                return Promise.all(ids.map( id => connection.query('DELETE FROM composite_material WHERE id = ?', [id])
                    .then( deleteInfo => deleteInfo.affectedRows)
                ))
                    .then( arrDeletedRows => arrDeletedRows.reduce((sum, deletedRows) => deletedRows + sum).toString())
            })
...
let query = 'DELETE FROM composite_material WHERE id = ?';
...
compositeMaterial: (pool, ids) => {
    ...
    // 1. Delete composite_has_material
}
*/
